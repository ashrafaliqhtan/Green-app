...
class OnboardingView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => OnboardingBloc(), // Initialize the OnboardingBloc
      child: BlocBuilder<OnboardingBloc, OnboardingState>(
        builder: (context, state) {
          final onboardingBloc = BlocProvider.of<OnboardingBloc>(context);
          bool isFirstPageButtonClicked =
              false; // Flag to track if the button on the first page has been clicked
          return Scaffold(
            body: PageView.builder(
              onPageChanged: (index) {
                if (state is! OnboardingLoadedState) {
                  onboardingBloc.add(
                      NextViewEvent()); // Emit NextViewEvent when page changes
                }
              },
                          ...
                          children: [
                            Container(
                              ...
                              child: TextButton(
                                onPressed: () {
                                  if (currentIndex == 0 &&
                                      !isFirstPageButtonClicked) {
                                    isFirstPageButtonClicked = true;
                                    context.push(
